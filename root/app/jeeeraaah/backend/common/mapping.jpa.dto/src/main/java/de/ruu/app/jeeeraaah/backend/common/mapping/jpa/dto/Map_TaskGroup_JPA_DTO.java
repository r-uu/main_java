...
@Mapper public interface Map_TaskGroup_JPA_DTO
{
	Map_TaskGroup_JPA_DTO INSTANCE = Mappers.getMapper(Map_TaskGroup_JPA_DTO.class);

	@Mapping(target = "name", ignore = true) // set in constructor
	@Mapping(target = "tasks", ignore = true) // tasks are mapped manually in afterMapping to use ObjectFactory
	@Mapping(target = "description" , ignore = true) // mapped in afterMapping
	@NonNull
	TaskGroupDTO map(@NonNull TaskGroupJPA in, @NonNull @Context ReferenceCycleTracking context);
			...
			)
	{
		...
		if (in.tasks().isPresent())
		{
			...
			if (persistenceUtil.isLoaded(in, TaskGroupJPA_.TASKS))
			{
				for (TaskJPA relatedTask : relatedTasks)
				{
					...
					if (isNull(relatedTaskMapped))
							...
							// Tasks are automatically added to the group via TaskDTO constructor,
							// so we only need to trigger the mapping to populate the context
							Map_Task_JPA_DTO.INSTANCE.map(relatedTask, context);
					// Note: No need to add task to group here - already done in TaskDTO constructor
				}
			}
		}
		// Map mutable fields
	 ...
	 */
	default void mapMutableFields(@NonNull TaskGroupJPA source, @NonNull TaskGroupDTO target)
	{
		target.description(source.description().orElse(null));
	}

	/** mapstruct object factory */
	@ObjectFactory default @NonNull TaskGroupDTO create(@NonNull TaskGroupJPA in)
	{
